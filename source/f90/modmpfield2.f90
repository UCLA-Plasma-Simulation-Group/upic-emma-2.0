...
!
!-----------------------------------------------------------------------
!      subroutine mppois2_init(ffc,ax,ay,affp,nx,ny,kstrt)                     ! M. Touati
      subroutine mppois2_init(ffc,ax,ay,affp,nx,ny,kstrt,kx,ky)                ! M. Touati
...
!
!-----------------------------------------------------------------------
!      subroutine mppois2(q,fxy,ffc,we,tfield,nx,ny,kstrt)                     ! M. Touati
      subroutine mppois2(q,fxy,ffc,we,tfield,nx,ny,kstrt,kx,ky)                ! M. Touati
...
!
!-----------------------------------------------------------------------
!      subroutine mpibpois2(cu,bxy,ffc,ci,wm,tfield,nx,ny,kstrt)               ! M. Touati
      subroutine mpibpois2(cu,bxy,ffc,ci,wm,tfield,nx,ny,kstrt,kx,ky)          ! M. Touati
...
! initialize timer
      call dtimer(dtime,itime,-1)      
! call low level procedure
!      call MIPPBPOISP23(cu,bxy,ffc,ci,wm,nx,ny,kstrt,nyv,kxp,  &
!     &nyhd)                                                                   ! M. Touati
      call MIPPBPOISP23(cu,bxy,ffc,ci,wm,nx,ny,kstrt,nyv,kxp,  &
     &nyhd,kx,ky)                                                              ! M. Touati
! record time
...
!
      subroutine mpmaxwel2_init(exyz,bxyz,exyz_corr,bxyz_corr,nye,kxp,nyh)
! This subroutine initialize the 4 particular modes : kx = 0     and ky = 0
!                                                     kx = pi/dx and ky = 0
!                                                     kx = 0     and ky = pi/dy
!                                                     kx = pi/dx and ky = pi/dy
		implicit none
! input
		integer, intent(in)                        :: nye, kxp, nyh
		complex, dimension(3,nye,kxp), intent(in)  :: exyz, bxyz
! output 
		complex, dimension(4,3), intent(out)       :: exyz_corr, bxyz_corr
! locals
		integer :: q1, dim
!
		q1 = nyh + 1
		do dim = 1,3,1
			! mode (kx,ky) = (0,0) :
			exyz_corr(1,dim) = cmplx(real(exyz(dim,1,1)),0.0)
			bxyz_corr(1,dim) = cmplx(real(bxyz(dim,1,1)),0.0)
			! mode (kx,ky) = (pi/delta_x,0) :
			exyz_corr(2,dim) = cmplx(aimag(exyz(dim,1,1)),0.0)
			bxyz_corr(2,dim) = cmplx(aimag(bxyz(dim,1,1)),0.0)
			! mode (kx,ky) = (0,pi/delta_y) :
			exyz_corr(3,dim) = cmplx(real(exyz(dim,q1,1)),0.0)
			bxyz_corr(3,dim) = cmplx(real(bxyz(dim,q1,1)),0.0)
			! mode (kx,ky) = (pi/delta_x,pi/delta_y) :
			exyz_corr(4,dim) = cmplx(aimag(exyz(dim,q1,1)),0.0)
			bxyz_corr(4,dim) = cmplx(aimag(bxyz(dim,q1,1)),0.0)
		end do
    end subroutine mpmaxwel2_init
!
!-----------------------------------------------------------------------
...
!
!-----------------------------------------------------------------------
      subroutine mplsremfield2(fxy,exy,tfield,nx,ny,kstrt)
! adds and smooths or copies and smooths complex vector fields in
! fourier space
      implicit none
      integer, intent(in) :: nx, ny, kstrt
      real, intent(inout) :: tfield
      complex, dimension(:,:,:), intent(in)    :: fxy
...
! initialize timer
      call dtimer(dtime,itime,-1)
! call low level procedure
      call MPPLSREMFIELD2(fxy,exy,nx,ny,kstrt,nyv,kxp)
! record time
      call dtimer(dtime,itime,1)
      tfield = tfield + real(dtime)
      end subroutine
...
!
!-----------------------------------------------------------------------
!      subroutine mpdcuperp2(dcu,amu,tfield,nx,ny,kstrt)                       ! M. Touati
      subroutine mpdcuperp2(dcu,amu,tfield,nx,ny,kstrt,kx,ky)                  ! M. Touati
...
! initialize timer
      call dtimer(dtime,itime,-1)
! call low level procedure
!      call MPPDCUPERP23(dcu,amu,nx,ny,kstrt,nyv,kxp)                          ! M. Touati
      call MPPDCUPERP23(dcu,amu,nx,ny,kstrt,nyv,kxp,kx,ky)                     ! M. Touati
! record time
      call dtimer(dtime,itime,1)
      tfield = tfield + real(dtime)
      end subroutine
