...
!     &mx,my,nxv,nypmx,mx1,mxyp1,ipbc)                                         ! M. Touati
      subroutine PPGJPPOST2L(ppart,cu,kpic,noff,qm,dt,nppmx,idimp,nx,ny,&
     &mx,my,nxv,nypmx,mx1,mxyp1,x,y,delta,ipbc)                                ! M. Touati
...
!!$OMP& ,dy,vx,vy,vz,scu)                                                      ! M. Touati
!$OMP PARALLEL DO
!$OMP& PRIVATE(i,j,k,noffp,moffp,nppp,nn,mm,mnoff,xx,yy,dxp,dyp,amx,
!$OMP& amy,dx,dy,vx,vy,vz,scu)                                                 ! M. Touati
      do 80 k = 1, mxyp1
      noffp = (k - 1)/mx1
      moffp = my*noffp
...
!      dx = amx*amy                                                            ! M. Touati
!      dy = dxp*amy                                                            ! M. Touati
	  dx = qm*amx*amy                                                          ! M. Touati
	  dy = qm*dxp*amy                                                          ! M. Touati
      vx = ppart(3,j,k)
      vy = ppart(4,j,k)
      vz = ppart(5,j,k)
...
!      dx = x + vx*dt                                                          ! M. Touati
!      dy = y + vy*dt                                                          ! M. Touati
      dx = xx + vx*dt                                                          ! M. Touati
      dy = yy + vy*dt                                                          ! M. Touati
...
!!$OMP& amy,dx,dy,vx,vy,vz,edgelx,edgely,edgerx,edgery,scu)                    ! M. Touati
!$OMP PARALLEL DO
!$OMP& PRIVATE(i,j,k,noffp,moffp,nppp,nn,mm,ih,nh,mnoff,xx,yy,dxp,dyp,
!$OMP& amx,amy,dx,dy,vx,vy,vz,edgelx,edgely,edgerx,edgery,scu)                 ! M. Touati
      do 90 k = 1, mxyp1
      noffp = (k - 1)/mx1
      moffp = my*noffp
      noffp = mx*(k - mx1*noffp - 1)
...
!      real x, y, dx, dy, vx, vy, vz, p2, gami                                 ! M. Touati
...
!!$OMP PARALLEL DO
!!$OMP& PRIVATE(i,j,k,noffp,moffp,nppp,nn,mm,mnoff,x,y,dxp,dyp,amx,amy,dx
!!$OMP& ,dy,vx,vy,vz,p2,gami,scu)                                              ! M. Touati
!$OMP PARALLEL DO
!$OMP& PRIVATE(i,j,k,noffp,moffp,nppp,nn,mm,mnoff,xx,yy,dxp,dyp,amx,
!$OMP& amy,dx,dy,vx,vy,vz,p2,gami,scu)                                         ! M. Touati
      do 80 k = 1, mxyp1
      noffp = (k - 1)/mx1
      moffp = my*noffp
      noffp = mx*(k - mx1*noffp - 1)
...
!!$OMP PARALLEL DO
!!$OMP& PRIVATE(i,j,k,noffp,moffp,nppp,nn,mm,ih,nh,mnoff,x,y,dxp,dyp,amx,
!!$OMP& amy,dx,dy,vx,vy,vz,edgelx,edgely,edgerx,edgery,p2,gami,scu)            ! M. Touati
!$OMP PARALLEL DO
!$OMP& PRIVATE(i,j,k,noffp,moffp,nppp,nn,mm,ih,nh,mnoff,xx,yy,dxp,dyp,
!$OMP& amx,amy,dx,dy,vx,vy,vz,edgelx,edgely,edgerx,edgery,p2,gami,scu)         ! M. Touati
      do 90 k = 1, mxyp1
      noffp = (k - 1)/mx1
      moffp = my*noffp
      noffp = mx*(k - mx1*noffp - 1)
...
! local data
      ...
      integer mnoff, i, j, k, nn, mm
      real dxp, dyp, amx, amy
!      real x, y, dx, dy, vx, vy, vz, v1, v2, v3, v4                           ! M. Touati
      real xx, yy, dx, dy, vx, vy, vz, v1, v2, v3, v4                          ! M. Touati
      real samu
...
!!$OMP& dx,dy,vx,vy,vz,v1,v2,v3,v4,samu)                                       ! M. Touati
!$OMP PARALLEL DO
...
!!$OMP PARALLEL DO
!!$OMP& PRIVATE(i,j,k,noffp,moffp,nppp,nn,mm,mnoff,x,y,dxp,dyp,amx,amy,  
!!$OMP& dx,dy,vx,vy,vz,v1,v2,v3,v4,p2,gami2,samu)                              ! M. Touati
!$OMP PARALLEL DO
!$OMP& PRIVATE(i,j,k,noffp,moffp,nppp,nn,mm,mnoff,xx,yy,dxp,dyp,amx,amy,  
!$OMP& dx,dy,vx,vy,vz,v1,v2,v3,v4,p2,gami2,samu)                               ! M. Touati
      do 80 k = 1, mxyp1
