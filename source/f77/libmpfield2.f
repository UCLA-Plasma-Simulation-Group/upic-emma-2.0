...
! IDFT[Ex](kpx(j),-ky(nyh+1)) = exy(1,k1,j)  (kpx(j) = kx(j+joff+1))
         ...
         fxy(2,k1,j) = zero
         !-----------------
C        wp = wp + at1*(q(k1,j)*conjg(q(k1,j)))
      endif
      sum1 = sum1 + wp
   50 continue
!$OMP END PARALLEL DO
...
! local data
      integer nxh, nyh, ny2, ks, joff, kxps, j, k, k1
      real dkx, dky, dkx2, at1
      complex zero, zt1
      complex cu_1_1, cu_1_k1, cu_j1_1, cu_j1_k1
      dimension cu_1_1(3), cu_1_k1(3), cu_j1_1(3), cu_j1_k1(3)
      nxh = nx/2
      nyh = max(1,ny/2)
      ny2 = ny + 2
      ks = kstrt - 1
...
!$OMP PARALLEL DO PRIVATE(j,k,k1,dkx,dkx2,dky,at1,zt1)
      ...
      if ((j+joff).gt.0) then
         ...
         k1 = ny2 - k
! ky > 0
! IDFT[jx](kpx(j),ky(k)) = cu(1,k,j) (kpx(j) = kx(j+joff+1))
         dky = ky(k1)
         ...
         cu(3,k,j) = cu(3,k,j)
! ky < 0
! IDFT[jx](kpx(j),-ky(k)) = cu(1,k1,j) (kpx(j) = kx(j+joff+1))
...
! mode numbers ky = 0, ny/2
         k1 = nyh + 1
! ky = 0
! IDFT{jx}(kpx(j),-ky(1)=0) = cu(1,1,j) (kpx(j)=kx(j+joff+1))
         cu(1,1,j) = zero
         cu(2,1,j) = cu(2,1,j)
         cu(3,1,j) = cu(3,1,j)
!ky = ny/2
! IDFT[jx](kpx(j),-ky(nyh+1)) = cu(1,k1,j)  (kpx(j) = kx(j+joff+1))
         cu(1,k1,j) = zero
         cu(2,k1,j) = zero
         cu(3,k1,j) = zero
      endif
...
! mode numbers kx = 0, nx/2
      if (ks.eq.0) then
         ...
         k1 = ny2 - k
! kx = 0
! IDFT[jx](kx(1)=0, ky(k)) = cu(1,k,1)
! IDFT[jx](kx(1)=0,-ky(k)) = conjg(cu(1,k,1))
         cu(1,k,1) = cu(1,k,1)
         cu(2,k,1) = zero
         cu(3,k,1) = cu(3,k,1)
! kx = nx / 2
! IDFT[jx](kx(nxh+1),-ky(k)) = cu(1,k1,1)
! IDFT[jx](kx(nxh+1), ky(k)) = cu(exy(1,k1,1))
         cu(1,k1,1) = zero
         cu(2,k1,1) = zero
         cu(3,k1,1) = zero
   30    continue
...
! kx = 0 and ky = ny/2
! IDTF[jx](kx(1)=0  ,-ky(nyh+1)) = cmplx( real(cu(1,k1,1)),0.0)
         cu_1_k1(1) = zero
         cu_1_k1(2) = zero
         cu_1_k1(3) = zero
! kx = nx/2 and ky = ny/2
! IDTF[Ex](kx(nxh+1),-ky(nyh+1)) = cmplx(aimag(exy(1,k1,1)),0.0)
         ...
         cu_j1_k1(3) = zero
! packed :
		 cu(1,k1,1) = cmplx(real(cu_1_k1(1)),real(cu_j1_k1(1)))
         cu(2,k1,1) = cmplx(real(cu_1_k1(2)),real(cu_j1_k1(2)))
         cu(3,k1,1) = cmplx(real(cu_1_k1(2)),real(cu_j1_k1(3)))
      endif
      return
      end
!-----------------------------------------------------------------------
...
! IDFT[jx](kx(nxh+1), ky(k)) = cu(exy(1,k1,1))
         dky = ky(k1)
         at1 = 1.0/(dky*dky + dkx2)
         zt1 = at1*(dkx*cu(1,k1,1) - dky*cu(2,k1,1))
         cu(1,k1,1) = cu(1,k1,1) - dkx*zt1
         cu(2,k1,1) = cu(2,k1,1) + dky*zt1
         cu(3,k1,j) = cu(3,k1,j)
   30    continue
         k1 = nyh + 1
! kx = ky = 0
...
! IDTF[Ex](kx(nxh+1),-ky(nyh+1)) = cmplx(aimag(exy(1,k1,1)),0.0)
         ...
         at1 = 1.0/(dky*dky + dkx2)
         zt1 = at1*(dkx*cu_j1_k1(1) - dky*cu_j1_k1(2))
         cu_j1_k1(1) = cu_j1_k1(1) - dkx*zt1
         cu_j1_k1(2) = cu_j1_k1(2) + dky*zt1
         cu_j1_k1(3) = cu_j1_k1(3)
! packed :
		 cu(1,k1,1) = cmplx(real(cu_1_k1(1)),real(cu_j1_k1(1)))
         cu(2,k1,1) = cmplx(real(cu_1_k1(2)),real(cu_j1_k1(2)))
         cu(3,k1,1) = cmplx(real(cu_1_k1(3)),real(cu_j1_k1(3)))
      endif
      return
      end
!-----------------------------------------------------------------------
!      subroutine MIPPBPOISP23(cu,bxy,ffc,ci,wm,nx,ny,kstrt,nyv,kxp,nyhd)      ! M. Touati
      subroutine MIPPBPOISP23(cu,bxy,ffc,ci,wm,                         &
     &nx,ny,kstrt,nyv,kxp,nyhd,kx,ky)                                          ! M. Touati
...
! bx(kx,ky) =  ci*sqrt(-1)*g(kx,ky)*ky*cuz(kx,ky),
! by(kx,ky) = -ci*sqrt(-1)*g(kx,ky)*kx*cuz(kx,ky),
! bz(kx,ky) =  ci*sqrt(-1)*g(kx,ky)*(kx*cuy(kx,ky)-ky*cux(kx,ky)),
! where kx = 2pi*j/nx, ky = 2pi*k/ny, and j,k = fourier mode numbers,
! g(kx,ky) = (affp/(kx**2+ky**2))*s(kx,ky),
! s(kx,ky) = exp(-((kx*ax)**2+(ky*ay)**2)/2), except for
...
! wm = nx*ny*sum((affp/(kx**2+ky**2+kz**2))*ci*ci
...
! local data
      ...
      complex bxy_1_1, bxy_j1_1, bxy_1_k1, bxy_j1_k1                           ! M. Touati
...
! IDFT[Ex](kpx(j),ky(k)) = exy(1,k,j) (kpx(j) = kx(j+joff+1))
         ...
         zt6 = bxy(3,k,j) - cdth*(dkx*zt2 - dky*zt3)
     ...
     &                  + zt9*conjg(zt9))
         zt4 = zt4 - cdth*(dky*zt1)
         zt5 = zt5 + cdth*(dkx*zt1)
         zt6 = zt6 - cdth*(dkx*zt2 - dky*zt3)
         bxy(1,k,j) = zt4
         bxy(2,k,j) = zt5
         bxy(3,k,j) = zt6
...
! IDFT[Ex](kpx(j),-ky(k)) = exy(1,k1,j) (kpx(j) = kx(j+joff+1))
         ...
         zt6 = bxy(3,k1,j) - cdth*(dkx*zt2 + dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt2 = cmplx(-aimag(zt5),real(zt5))
         zt3 = cmplx(-aimag(zt4),real(zt4))
...
! packing :
      ...
      sum2 = sum2 + wp
   40 continue
      wf = real(nx)*real(ny)*sum1 ! *(1/2) * 2 where * 2 accounts for the negative kx
      wm = real(nx)*real(ny)*sum2 ! *(1/2) * 2 where * 2 accounts for the negative kx
      return
      end
...
! PSATD
      ...
      real cdt_corr, cdth_corr ! correction needed for PSATD 
...
! IDFT[Ex](kpx(j),ky(k)) = exy(1,k,j) (kpx(j) = kx(j+joff+1))
         ...
         zt2 = cmplx(-aimag(exy(2,k,j)),real(exy(2,k,j)))
         zt3 = cmplx(-aimag(exy(1,k,j)),real(exy(1,k,j)))
         zt4 = c_h * bxy(1,k,j) - cdth_corr*(dky*zt1)
         zt5 = c_h * bxy(2,k,j) + cdth_corr*(dkx*zt1)
         zt6 = c_h * bxy(3,k,j) - cdth_corr*(dkx*zt2 - dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt2 = cmplx(-aimag(zt5),real(zt5))
         zt3 = cmplx(-aimag(zt4),real(zt4))
         zt7 = c_h * exy(1,k,j) + cdt_corr*(dky*zt1)                    &
     &   - afdt*cu(1,k,j)
         zt8 = c_h * exy(2,k,j) - cdt_corr*(dkx*zt1)                    &
     &   - afdt*cu(2,k,j)
         zt9 = c_h * exy(3,k,j) + cdt_corr*(dkx*zt2 - dky*zt3)          &
     &   - afdt*cu(3,k,j)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt2 = cmplx(-aimag(zt8),real(zt8))
         ...
         ws = ws + anorm*(zt7*conjg(zt7) + zt8*conjg(zt8)               &
     &                  + zt9*conjg(zt9))
         zt4 = zt4 - cdth_corr*(dky*zt1)
         zt5 = zt5 + cdth_corr*(dkx*zt1)
         zt6 = zt6 - cdth_corr*(dkx*zt2 - dky*zt3)
         bxy(1,k,j) = zt4
         bxy(2,k,j) = zt5
         bxy(3,k,j) = zt6
         wp = wp + anorm*(zt4*conjg(zt4) + zt5*conjg(zt5)               &
...
! IDFT[Ex](kpx(j),-ky(k)) = exy(1,k1,j) (kpx(j) = kx(j+joff+1))
         ...
         zt2 = cmplx(-aimag(exy(2,k1,j)),real(exy(2,k1,j)))
         zt3 = cmplx(-aimag(exy(1,k1,j)),real(exy(1,k1,j)))
         zt4 = c_h * bxy(1,k1,j) + cdth_corr*(dky*zt1)
         zt5 = c_h * bxy(2,k1,j) + cdth_corr*(dkx*zt1)
         zt6 = c_h * bxy(3,k1,j) - cdth_corr*(dkx*zt2 + dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt2 = cmplx(-aimag(zt5),real(zt5))
         zt3 = cmplx(-aimag(zt4),real(zt4))
         zt7 = c_h * exy(1,k1,j) - cdt_corr*(dky*zt1)                   &
     &   - afdt*cu(1,k1,j)
         zt8 = c_h * exy(2,k1,j) - cdt_corr*(dkx*zt1)                   &
     &   - afdt*cu(2,k1,j)
         zt9 = c_h * exy(3,k1,j) + cdt_corr*(dkx*zt2 + dky*zt3)         &
     &   - afdt*cu(3,k1,j)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt2 = cmplx(-aimag(zt8),real(zt8))
...
! IDFT{Ex}(kpx(j),-ky(1)=0) = exy(1,1,j) (kpx(j)=kx(j+joff+1))
         ...
         zt1 = cmplx(-aimag(exy(3,1,j)),real(exy(3,1,j)))
         zt2 = cmplx(-aimag(exy(2,1,j)),real(exy(2,1,j)))
         zt5 = c_h * bxy(2,1,j) + cdth_corr*(dkx*zt1)
         zt6 = c_h * bxy(3,1,j) - cdth_corr*(dkx*zt2)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt2 = cmplx(-aimag(zt5),real(zt5))
         zt8 = c_h * exy(2,1,j) - cdt_corr*(dkx*zt1) - afdt*cu(2,1,j)
         zt9 = c_h * exy(3,1,j) + cdt_corr*(dkx*zt2) - afdt*cu(3,1,j)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt2 = cmplx(-aimag(zt8),real(zt8))
         exy(1,1,j) = zero
...
! IDFT[Ex](kx(1)=0,-ky(k)) = conjg(exy(1,k,1))
         ...
         zt1 = cmplx(-aimag(exy(3,k,1)),real(exy(3,k,1)))
         zt3 = cmplx(-aimag(exy(1,k,1)),real(exy(1,k,1)))
         zt4 = c_h * bxy(1,k,1) - cdth_corr*(dky*zt1)
         zt6 = c_h * bxy(3,k,1) + cdth_corr*(dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt3 = cmplx(-aimag(zt4),real(zt4))
         zt7 = c_h * exy(1,k,1) + cdt_corr*(dky*zt1) - afdt*cu(1,k,1)
         zt9 = c_h * exy(3,k,1) - cdt_corr*(dky*zt3) - afdt*cu(3,k,1)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt3 = cmplx(-aimag(zt7),real(zt7))
         exy(1,k,1) = zt7
...
! PSATD corrections here
         ...
         bxy_corr(2,1) = zero
         bxy_corr(2,2) = c_h * bxy_corr(2,2) + cdth_corr *(dkx*zt1)
         bxy_corr(2,3) = c_h * bxy_corr(2,3) - cdth_corr *(dkx*zt2)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(bxy_corr(2,3)),real(bxy_corr(2,3)))
         zt2 = cmplx(-aimag(bxy_corr(2,2)),real(bxy_corr(2,2)))
         exy_corr(2,1) = zero
         exy_corr(2,2) = c_h * exy_corr(2,2) - cdt_corr *               &
     &   (dkx*zt1) - afdt*cu_temp(2)
         exy_corr(2,3) = c_h * exy_corr(2,3) + cdt_corr *               &
     &   (dkx*zt2) - afdt*cu_temp(3)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(exy_corr(2,3)),real(exy_corr(2,3)))
         zt2 = cmplx(-aimag(exy_corr(2,2)),real(exy_corr(2,2)))
     ...
     &                  + zt9*conjg(zt9))
         bxy_corr(2,1) = zero
         bxy_corr(2,2) = c_h * bxy_corr(2,2) + cdt_corr*(dkx*zt1)
         bxy_corr(2,3) = c_h * bxy_corr(2,3) - cdt_corr*(dkx*zt2)
...
! PSATD corrections here
         ...
         bxy_corr(3,1) = c_h * bxy_corr(3,1) + cdth_corr*(dky*zt1)
         bxy_corr(3,2) = zero
         bxy_corr(3,3) = c_h * bxy_corr(3,3) - cdth_corr*(dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(bxy_corr(3,3)),real(bxy_corr(3,3)))
         zt3 = cmplx(-aimag(bxy_corr(3,1)),real(bxy_corr(3,1)))
         exy_corr(3,1) = c_h * exy_corr(3,1) - cdt_corr*(dky*zt1) -           &
     &   afdt*cu_temp(1)
         exy_corr(3,2) = zero
         exy_corr(3,3) = c_h * exy_corr(3,3) + cdt_corr*(dky*zt3) -           &
     &   afdt*cu_temp(3)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(exy_corr(3,3)),real(exy_corr(3,3)))
         zt3 = cmplx(-aimag(exy_corr(3,1)),real(exy_corr(3,1)))
         ...
         ws = ws + anorm*(zt7*conjg(zt7) + zt8*conjg(zt8)               &
     &                  + zt9*conjg(zt9))
         bxy_corr(3,1) = c_h * bxy_corr(3,1) + cdt_corr*(dky*zt1)
         bxy_corr(3,2) = zero
         bxy_corr(3,3) = c_h * bxy_corr(3,3) - cdt_corr*(dky*zt3)
...
! PSATD corrections here
         ...
         exy_corr(4,1) = c_h *exy_corr(4,1) - cdt_corr*(dky*zt1)        &
     &                 - afdt*cu_temp(1)
         exy_corr(4,2) = c_h * exy_corr(4,2) - cdt_corr*(dkx*zt1)       &
     &                 - afdt*cu_temp(2)
         exy_corr(4,3) = c_h * exy_corr(4,3) +                          &
     &   cdt_corr*(dkx*zt2 + dky*zt3)                                   &
     &                 - afdt*cu_temp(3)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(exy_corr(4,3)),real(exy_corr(4,3)))
         zt2 = cmplx(-aimag(exy_corr(4,2)),real(exy_corr(4,2)))
...
!-----------------------------------------------------------------------
      subroutine MPPMAXWEL2YEE_PSATD(exy,bxy,exy_corr,bxy_corr,cu,ffc,        &
     &affp,ci,dt,wf,wm,nx,ny,kstrt,nyv,kxp,nyhd,kx,ky,ax,ay)               
! this subroutine solves 2d maxwell's equation in fourier space for
! transverse electric and magnetic fields with periodic boundary
! conditions.
...
! nyhd = first dimension of form factor array, must be >= nyh
      ...
      integer nx, ny, kstrt, nyv, kxp, nyhd
      real affp, ci, dt, wf, wm, ax, ay
      complex exy, bxy, cu, ffc, exy_corr, bxy_corr                           
      dimension exy(3,nyv,kxp), bxy(3,nyv,kxp), cu(3,nyv,kxp)
      dimension ffc(nyhd,kxp), exy_corr(4,3), bxy_corr(4,3)
      real kx, ky                                                              
      dimension kx(nx+2), ky(ny+2)  
      real omega_corr
      dimension omega_corr(nx+2,ny+2)                                          
! local data
      integer nxh, nyh, ny2, ks, joff, kxps, j, k, k1
      real cdth, cdt, adt, anorm, dkx, dky, afdt
! PSATD
...
! PSATD
      ...
      dimension bxy_1_1(3), bxy_j1_1(3), bxy_1_k1(3), bxy_j1_k1(3)             
      complex exy_1_1, exy_j1_1, exy_1_k1, exy_j1_k1                           
      dimension exy_1_1(3), exy_j1_1(3), exy_1_k1(3), exy_j1_k1(3)             
      complex  cu_1_1,  cu_j1_1,  cu_1_k1,  cu_j1_k1                           
      dimension  cu_1_1(3),  cu_j1_1(3),  cu_1_k1(3),  cu_j1_k1(3)             
      if (ci.le.0.0) return
      nxh = nx/2
      nyh = max(1,ny/2)
      ny2 = ny + 2
...
! IDFT[Ex](kpx(j),ky(k)) = exy(1,k,j) (kpx(j) = kx(j+joff+1))
         ...
         zt2 = cmplx(-aimag(exy(2,k,j)),real(exy(2,k,j)))
         zt3 = cmplx(-aimag(exy(1,k,j)),real(exy(1,k,j)))
         zt4 = bxy(1,k,j) - cdth_corr*(dky*zt1)
         zt5 = bxy(2,k,j) + cdth_corr*(dkx*zt1)
         zt6 = bxy(3,k,j) - cdth_corr*(dkx*zt2 - dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt2 = cmplx(-aimag(zt5),real(zt5))
         zt3 = cmplx(-aimag(zt4),real(zt4))
!23456789*123456789*123456789*123456789*123456789*123456789*123456789*12345
         zt7 = exy(1,k,j) + cdt_corr*(dky*zt1)                          &
     &   - afdt*cu(1,k,j)
         zt8 = exy(2,k,j) - cdt_corr*(dkx*zt1)                          &
     &   - afdt*cu(2,k,j)
         zt9 = exy(3,k,j) + cdt_corr*(dkx*zt2 - dky*zt3)                & 
     &   - afdt*cu(3,k,j)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt2 = cmplx(-aimag(zt8),real(zt8))
...
! IDFT[Ex](kpx(j),-ky(k)) = exy(1,k1,j) (kpx(j) = kx(j+joff+1))
         ...
         zt2 = cmplx(-aimag(exy(2,k1,j)),real(exy(2,k1,j)))
         zt3 = cmplx(-aimag(exy(1,k1,j)),real(exy(1,k1,j)))
         zt4 = bxy(1,k1,j) + cdth_corr*(dky*zt1)
         zt5 = bxy(2,k1,j) + cdth_corr*(dkx*zt1)
         zt6 = bxy(3,k1,j) - cdth_corr*(dkx*zt2 + dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt2 = cmplx(-aimag(zt5),real(zt5))
         zt3 = cmplx(-aimag(zt4),real(zt4))
!23456789*123456789*123456789*123456789*123456789*123456789*123456789*12345
         zt7 = exy(1,k1,j) - cdt_corr*(dky*zt1)           -             &
     &   afdt*cu(1,k1,j)
         zt8 = exy(2,k1,j) - cdt_corr*(dkx*zt1)           -             &
     &   afdt*cu(2,k1,j)
         zt9 = exy(3,k1,j) + cdt_corr*(dkx*zt2 + dky*zt3) -             &
     &   afdt*cu(3,k1,j)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt2 = cmplx(-aimag(zt8),real(zt8))
...
! IDFT{Ex}(kpx(j),-ky(1)=0) = exy(1,1,j) (kpx(j)=kx(j+joff+1))
         ...
         zt1 = cmplx(-aimag(exy(3,1,j)),real(exy(3,1,j)))
         zt2 = cmplx(-aimag(exy(2,1,j)),real(exy(2,1,j)))
         zt5 = bxy(2,1,j) + cdth_corr*(dkx*zt1)
         zt6 = bxy(3,1,j) - cdth_corr*(dkx*zt2)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt2 = cmplx(-aimag(zt5),real(zt5))
         zt8 = exy(2,1,j) - cdt_corr*(dkx*zt1) - afdt*cu(2,1,j)
         zt9 = exy(3,1,j) + cdt_corr*(dkx*zt2) - afdt*cu(3,1,j)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt2 = cmplx(-aimag(zt8),real(zt8))
         exy(1,1,j) = zero
...
! IDFT[Ex](kx(1)=0,-ky(k)) = conjg(exy(1,k,1))
         ...
         zt1 = cmplx(-aimag(exy(3,k,1)),real(exy(3,k,1)))
         zt3 = cmplx(-aimag(exy(1,k,1)),real(exy(1,k,1)))
         zt4 = bxy(1,k,1) - cdth_corr*(dky*zt1)
         zt6 = bxy(3,k,1) + cdth_corr*(dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt3 = cmplx(-aimag(zt4),real(zt4))
         zt7 = exy(1,k,1) + cdt_corr*(dky*zt1) - afdt*cu(1,k,1)
         zt9 = exy(3,k,1) - cdt_corr*(dky*zt3) - afdt*cu(3,k,1)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt3 = cmplx(-aimag(zt7),real(zt7))
         exy(1,k,1) = zt7
...
! IDFT[Ex](kx(nxh+1), ky(k)) = conjg(exy(1,k1,1))
         k1 = ny2 - k
         ! update magnetic field half time step
         zt1 = cmplx(-aimag(exy(3,k1,1)),real(exy(3,k1,1)))
         zt2 = cmplx(-aimag(exy(2,k1,1)),real(exy(2,k1,1)))
         zt3 = cmplx(-aimag(exy(1,k1,1)),real(exy(1,k1,1)))
         zt4 = bxy(1,k1,1) + cdth_corr*(dky*zt1)
         zt5 = bxy(2,k1,1) + cdth_corr*(dkx*zt1)
         zt6 = bxy(3,k1,1) - cdth_corr*(dkx*zt2 + dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt2 = cmplx(-aimag(zt5),real(zt5))
         zt3 = cmplx(-aimag(zt4),real(zt4))
!23456789*123456789*123456789*123456789*123456789*123456789*123456789*12345
         zt7 = exy(1,k1,1) - cdt_corr*(dky*zt1)                         &
     &   - afdt*cu(1,k1,1)
         zt8 = exy(2,k1,1) - cdt_corr*(dkx*zt1)                         &
     &   - afdt*cu(2,k1,1)
         zt9 = exy(3,k1,1) + cdt_corr*(dkx*zt2 + dky*zt3)               &
     &   - afdt*cu(3,k1,1)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt2 = cmplx(-aimag(zt8),real(zt8))
...
! IDTF[Ex](kx(1)=0  ,-ky(1)=0) = cmplx(real(exy(1,1,1)),0.0)
         ...
         ! update electric field whole time step
         exy_corr(1,1) = exy_corr(1,1) - afdt*cu_1_1(1)
         exy_corr(1,2) = exy_corr(1,2) - afdt*cu_1_1(2)
         exy_corr(1,3) = exy_corr(1,3) - afdt*cu_1_1(3)
         ! update magnetic field half time step and store electric field
         bxy_corr(1,1) = bxy_corr(1,1)
         bxy_corr(1,2) = bxy_corr(1,2)
         bxy_corr(1,3) = bxy_corr(1,3)
...
! IDFT[Ex](kx(nxh+1),-ky(1)=0)   = cmplx(aimag(exy(1,1,1)),0.0)
         ...
         exy_corr(2,1) = zero
!23456789*123456789*123456789*123456789*123456789*123456789*123456789*12345
         exy_corr(2,2) = exy_corr(2,2) - cdt_corr*(dkx*zt1)             &
     &	 - afdt*cu_j1_1(2)
         exy_corr(2,3) = exy_corr(2,3) + cdt_corr*(dkx*zt2)             &
     &	 - afdt*cu_j1_1(3)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(exy_corr(2,3)),real(exy_corr(2,3)))
         zt2 = cmplx(-aimag(exy_corr(2,2)),real(exy_corr(2,2)))
...
! IDTF[Ex](kx(1)=0  ,-ky(nyh+1)) = cmplx( real(exy(1,k1,1)),0.0)
         ...
         zt1 = cmplx(-aimag(exy_corr(3,3)),real(exy_corr(3,3)))
         zt3 = cmplx(-aimag(exy_corr(3,1)),real(exy_corr(3,1)))
         bxy_corr(3,1) = bxy_corr(3,1) + cdth_corr*(dky*zt1)
         bxy_corr(3,2) = zero
         bxy_corr(3,3) = bxy_corr(3,3) - cdth_corr*(dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(bxy_corr(3,3)),real(bxy_corr(3,3)))
         zt3 = cmplx(-aimag(bxy_corr(3,1)),real(bxy_corr(3,1)))
!23456789*123456789*123456789*123456789*123456789*123456789*123456789*12345
         exy_corr(3,1) = exy_corr(3,1) - cdt_corr*(dky*zt1)             &
     &   - afdt*cu_1_k1(1)
         exy_corr(3,2) = zero
         exy_corr(3,3) = exy_corr(3,3) + cdt_corr*(dky*zt3)             &
     &   - afdt*cu_1_k1(3)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(exy_corr(3,3)),real(exy_corr(3,3)))
         zt3 = cmplx(-aimag(exy_corr(3,1)),real(exy_corr(3,1)))
...
! IDTF[Ex](kx(nxh+1),-ky(nyh+1)) = cmplx(aimag(exy(1,k1,1)),0.0)
         ...
         zt2 = cmplx(-aimag(exy_corr(4,2)),real(exy_corr(4,2)))
         zt3 = cmplx(-aimag(exy_corr(4,1)),real(exy_corr(4,1)))
         bxy_corr(4,1) = bxy_corr(4,1) + cdth_corr*(dky*zt1)
         bxy_corr(4,2) = bxy_corr(4,2) + cdth_corr*(dkx*zt1)
         bxy_corr(4,3) = bxy_corr(4,3) - cdth_corr*(dkx*zt2 + dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(bxy_corr(4,3)),real(bxy_corr(4,3)))
         zt2 = cmplx(-aimag(bxy_corr(4,2)),real(bxy_corr(4,2)))
         zt3 = cmplx(-aimag(bxy_corr(4,1)),real(bxy_corr(4,1)))
         exy_corr(4,1) = exy_corr(4,1) - cdt_corr*(dky*zt1)                  &
     &                 - afdt*cu_j1_k1(1)
         exy_corr(4,2) = exy_corr(4,2) - cdt_corr*(dkx*zt1)                  &
     &                 - afdt*cu_j1_k1(2)
         exy_corr(4,3) = exy_corr(4,3) + cdt_corr*(dkx*zt2 + dky*zt3)        &
     &                 - afdt*cu_j1_k1(3)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(exy_corr(4,3)),real(exy_corr(4,3)))
         zt2 = cmplx(-aimag(exy_corr(4,2)),real(exy_corr(4,2)))
...
! nyhd = first dimension of form factor array, must be >= nyh
      ...
      integer nx, ny, kstrt, nyv, kxp, nyhd
      real affp, ci, dt, wf, wm, ax, ay
      complex exy, bxy, cu, ffc, exy_corr, bxy_corr                            ! M. Touati
      dimension exy(3,nyv,kxp), bxy(3,nyv,kxp), cu(3,nyv,kxp)
      dimension ffc(nyhd,kxp), exy_corr(4,3), bxy_corr(4,3)
      real kx, ky                                                              ! M. Touati
      dimension kx(nx+2), ky(ny+2)                                             ! M. Touati
! local data
      ...
      complex exy_1_1, exy_j1_1, exy_1_k1, exy_j1_k1                           ! M. Touati
      dimension exy_1_1(3), exy_j1_1(3), exy_1_k1(3), exy_j1_k1(3)             ! M. Touati
      complex  cu_1_1,  cu_j1_1,  cu_1_k1,  cu_j1_k1                           ! M. Touati
      dimension  cu_1_1(3),  cu_j1_1(3),  cu_1_k1(3),  cu_j1_k1(3)             ! M. Touati
      if (ci.le.0.0) return
      nxh = nx/2
      nyh = max(1,ny/2)
      ny2 = ny + 2
...
! IDFT[Ex](kx(nxh+1), ky(k)) = conjg(exy(1,k1,1))
         ...
         zt6 = bxy(3,k1,1) - cdth*(dkx*zt2 + dky*zt3)
         ! update electric field whole time step
         zt1 = cmplx(-aimag(zt6),real(zt6))
         zt2 = cmplx(-aimag(zt5),real(zt5))
         zt3 = cmplx(-aimag(zt4),real(zt4))
         ...
         zt9 = exy(3,k1,1) + cdt*(dkx*zt2 + dky*zt3) - afdt*cu(3,k1,1)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(zt9),real(zt9))
         zt2 = cmplx(-aimag(zt8),real(zt8))
         zt3 = cmplx(-aimag(zt7),real(zt7))
...
! IDFT[Ex](kx(nxh+1),-ky(1)=0)   = cmplx(aimag(exy(1,1,1)),0.0)
         ...
         zt2 = cmplx(-aimag(bxy_corr(2,2)),real(bxy_corr(2,2)))
         exy_corr(2,1) = zero
         exy_corr(2,2) = exy_corr(2,2) - cdt*(dkx*zt1) - afdt*cu_j1_1(2)
         exy_corr(2,3) = exy_corr(2,3) + cdt*(dkx*zt2) - afdt*cu_j1_1(3)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(exy_corr(2,3)),real(exy_corr(2,3)))
         zt2 = cmplx(-aimag(exy_corr(2,2)),real(exy_corr(2,2)))
         zt7 = cmplx(real(exy_corr(2,1)),0.0)
...
! IDTF[Ex](kx(1)=0  ,-ky(nyh+1)) = cmplx( real(exy(1,k1,1)),0.0)
         ...
         zt1 = cmplx(-aimag(bxy_corr(3,3)),real(bxy_corr(3,3)))
         zt3 = cmplx(-aimag(bxy_corr(3,1)),real(bxy_corr(3,1)))
         exy_corr(3,1) = exy_corr(3,1) - cdt*(dky*zt1) - afdt*cu_1_k1(1)
         exy_corr(3,2) = zero
         exy_corr(3,3) = exy_corr(3,3) + cdt*(dky*zt3) - afdt*cu_1_k1(3)
         ! update magnetic field half time step and store electric field
         zt1 = cmplx(-aimag(exy_corr(3,3)),real(exy_corr(3,3)))
         zt3 = cmplx(-aimag(exy_corr(3,1)),real(exy_corr(3,1)))
         zt7 = cmplx(real(exy_corr(3,1)),0.0)
...
! includes additional smoothing
      ...
      dimension kx(nx+2), ky(ny+2)                                             ! M. Touati
! local data
	  complex re_f, re_e, im_f, im_e
	  dimension re_f(3), re_e(3), im_f(3), im_e(3)
      integer i, nxh, nyh, ny2, ks, joff, kxps, j, k, k1
      real at1, at2, dkx, dky
      nxh = nx/2
      nyh = max(1,ny/2)
      ny2 = ny + 2
      ks = kstrt - 1
...
! add the fields
      if (isign.gt.0) then
! mode numbers 0 < kx < nx/2 and 0 < ky < ny/2
!$OMP PARALLEL DO PRIVATE(i,j,k,k1,at1,at2,dkx,dky)
         do 40 j = 1, kxps
         if ((j+joff).gt.0) then
         do 20 k = 2, nyh
         k1 = ny2 - k
         at1 = aimag(ffc(k,j))
         do 10 i = 1, 3
! exy(1,k,j) = IDFT{Ex}(kpx(j), ky(k)) (kpx(j)=kx(j+joff+1))
         fxy(i,k ,j) = fxy(i,k ,j) + exy(i,k, j)*at1
! exy(1,k1,j) = IDFT{Ex}(kpx(j),-ky(k)) (kpx(j)=kx(j+joff+1))
         fxy(i,k1,j) = fxy(i,k1,j) + exy(i,k1,j)*at1
   10    continue
   20    continue
! mode numbers ky = 0, ny/2
		 ...
		 dkx = kx(j+joff+1)
		 dky = ky(nyh+1)
         at2 = exp(-0.5*(((ax*dkx)**2)+((ay*dky)**2)))
         do 30 i = 1, 3
! exy(1,1,j) = IDFT{Ex}(kpx(j),-ky(1)=0)  (kpx(j)=kx(j+joff+1))
         fxy(i, 1,j) = fxy(i, 1,j) + exy(i, 1,j)*at1
! exy(1,k1,j) = IDFT{Ex}(kpx(j),-ky(ny/2))  (kpx(j)=kx(j+joff+1))
         fxy(i,k1,j) = fxy(i,k1,j) + exy(i,k1,j)*at2
   30    continue
         end if
   40    continue
         !$OMP END PARALLEL DO
         ...
         dkx = kx(nxh+1)
         do 60 k = 2, nyh
! modes numbers kx = 0, nx/2
         k1 = ny2 - k
         at1 = aimag(ffc(k,1))
         dky = ky(k)
         at2 = exp(-0.5*(((ax*dkx)**2)+((ay*dky)**2)))
         do 50 i = 1,3
! exy(1,k,1) = IDFT{Ex}(kx(1)=0, ky(k))
         fxy(i, k,1) = fxy(i, k,1) + exy(i, k,1)*at1
! exy(1,k1,1) = IDFT{Ex}(kx(nx/2),-ky(k))
         fxy(i,k1,1) = fxy(i,k1,1) + exy(i,k1,1)*at2
   50    continue
   60    continue
		 k1 = nyh + 1
		 dky = ky(nyh+1)
         do 70 i=1,3
! modes numbers (kx,ky) = (0,0), (nx/2,0), (0,ny/2) and (nx/2,ny/2)
! IDFT{Ex}(kx(1)=0  ,-ky(1)=0) = cmplx( real(exy(1,1,1)),0.0)
         re_f(i) = cmplx(real(fxy(i,1,1)),0.0)
         re_e(i) = cmplx(real(exy(i,1,1)),0.0)
         re_f(i) = re_f(i) + re_e(i)
! IDFT[Bx](kx(nxh+1),-ky(1)=0) = cmplx(aimag(bxy(1,1,1)),0.0)
         at2 = exp(-0.5*((ax*dkx)**2))
         im_f(i) = cmplx(aimag(fxy(i,1,1)),0.0)
         im_e(i) = cmplx(aimag(exy(i,1,1)),0.0)
         im_f(i) = im_f(i) + im_e(i)*at2
! repacking :
		 fxy(i,1,1) = cmplx(real(re_f(i)),real(im_f(i)))
! modes kx = 0 , ky(ny/2)
! IDFT{Ex}(kx(1)=0  ,-ky(nyh+1)) = cmplx( real(exy(1,k1,1)),0.0)
		 at1 = exp(-0.5*((ay*dky)**2))
		 re_f(i) = cmplx(real(fxy(i,k1,1)),0.0)
		 re_e(i) = cmplx(real(exy(i,k1,1)),0.0)
		 re_f(i) = re_f(i) + re_e(i)*at1
! IDTF[Bx](kx(nxh+1),-ky(nyh+1)) = cmplx(aimag(bxy(1,k1,1)),0.0)
         at2 = exp(-0.5*(((ax*dkx)**2)+((ay*dky)**2)))
		 im_f(i) = cmplx(aimag(fxy(i,k1,1)),0.0)
		 im_e(i) = cmplx(aimag(exy(i,k1,1)),0.0)
		 im_f(i) = im_f(i) + im_e(i)*at2
! repacking :
		 fxy(i,k1,1) = cmplx(real(re_f(i)),real(im_f(i)))           
   70    continue
         end if
! copy the fields
      else if (isign.lt.0) then
...
! modes numbers kx = 0, nx/2
         ...
         do 120 i = 1,3
! exy(1,k,1) = IDFT{Ex}(kx(1)=0, ky(k))
         fxy(i, k,1) = exy(i, k,1)*at1
! exy(1,k1,1) = IDFT{Ex}(kx(nx/2),-ky(k))
         fxy(i,k1,1) = exy(i,k1,1)*at2
  120    continue
  140    continue
		 k1 = nyh + 1
		 dky = ky(nyh+1)
...
! this subroutine adds complex vector fields if isign > 0
      implicit none
      integer isign, nx, ny, kstrt, nyv, kxp
      complex fxy, exy, ffc
      dimension fxy(3,nyv,kxp), exy(3,nyv,kxp)
! local data
      integer i, nxh, nyh, ny2, ks, joff, kxps, j, k, k1
...
! add the fields
      ...
      k1 = ny2 - k
      do 10 i = 1, 3
      exy(i,k,j)  = exy(i,k,j)  + fxy(i,k,j)
      exy(i,k1,j) = exy(i,k1,j) + fxy(i,k1,j)
   10 continue
   20 continue
      ! mode numbers ky = 0, ny/2
      k1 = nyh + 1
      do 30 i = 1, 3
      exy(i,1,j)  = exy(i,1,j)  + fxy(i,1,j)
      exy(i,k1,j) = exy(i,k1,j) + fxy(i,k1,j)
   30 continue
   40 continue
      !$OMP END PARALLEL DO
...
! local data
      integer nxh, nyh, ny2, ks, joff, kxps, j, k, k1
      real dkx, dky, dkx2, dky2, dkxy, dkxy2, at1
      complex zero, zt1, zt2, zt3
      complex dcu_1_1, dcu_1_k1, dcu_j1_1, dcu_j1_k1
      dimension dcu_1_1(3), dcu_1_k1(3), dcu_j1_1(3), dcu_j1_k1(3)
      nxh = nx/2
      nyh = max(1,ny/2)
      ny2 = ny + 2
      ks = kstrt - 1
...
!$OMP& PRIVATE(j,k,k1,dkx,dkx2,dky,dky2,dkxy,dkxy2,at1,zt1,zt2,zt3)
      ...
      if ((j+joff).gt.0) then
         ...
         at1 = 1.0/(dkx2 + dky2)
! ky > 0
! IDFT[djx/dt](kpx(j),ky(k)) = amu(1,k,j) (kpx(j) = kx(j+joff+1))
...
! mode numbers ky = 0, ny/2
         k1 = nyh + 1
! ky = 0
! IDFT{djx/dt}(kpx(j),-ky(1)=0) = amu(1,1,j) (kpx(j)=kx(j+joff+1))
         ...
         dcu(3,1,j) = dkx*zt1
! ky = ny / 2
! IDFT{djx/dt}(kpx(j),-ky(nyh+1)=0) = amu(1,1,j) (kpx(j)=kx(j+joff+1))
         dcu(1,k1,j) = zero
         dcu(2,k1,j) = zero
         dcu(3,k1,j) = zero
      endif
...
! mode numbers kx = 0, nx/2
      if (ks.eq.0) then
         ...
         k1 = ny2 - k
! kx = 0
! IDFT[djx/dt](kx(1)=0, ky(k)) = amu(1,k,1)
! IDFT[djx/dt](kx(1)=0,-ky(k)) = conjg(amu(1,k,1))
         dky = ky(k)
         ...
         dcu(3,k,1) = dky*zt2
! kx = nx / 2
! IDFT[djx/dt](kx(nxh+1),-ky(k)) = amu(1,k1,1)
! IDFT[djx/dt](kx(nxh+1), ky(k)) = amu(exy(1,k1,1))
         dcu(1,k1,1) = zero
         dcu(2,k1,1) = zero
         dcu(3,k1,1) = zero
   30    continue
         k1 = nyh + 1
! kx = ky = 0
! IDTF[djx/dt](kx(1)=0  ,-ky(1)=0) = cmplx(real(amu(1,1,1)),0.0)
		 dcu_1_1(1) = zero
		 dcu_1_1(2) = zero
		 dcu_1_1(3) = zero
! kx = nx/2 and ky = 0
! IDFT[jx](kx(nxh+1),-ky(1)=0)   = cmplx(aimag(cu(1,1,1)),0.0)
         ...
         dcu_j1_1(3) = zero
! packed :
		 dcu(1,1,1) = cmplx(real(dcu_1_1(1)),real(dcu_j1_1(1)))
         dcu(2,1,1) = cmplx(real(dcu_1_1(2)),real(dcu_j1_1(2)))
         dcu(2,1,1) = cmplx(real(dcu_1_1(3)),real(dcu_j1_1(3)))
! kx = 0 and ky = ny/2
! IDTF[jx](kx(1)=0  ,-ky(nyh+1)) = cmplx( real(cu(1,k1,1)),0.0)
         dcu_1_k1(1) = zero
         dcu_1_k1(2) = zero
...
! IDTF[Ex](kx(nxh+1),-ky(nyh+1)) = cmplx(aimag(exy(1,k1,1)),0.0)
         dcu_j1_k1(1) = zero
 	     ...
 	     dcu_j1_k1(3) = zero
! packed :
		 dcu(1,k1,1) = cmplx(real(dcu_1_k1(1)),real(dcu_j1_k1(1)))
         dcu(2,k1,1) = cmplx(real(dcu_1_k1(2)),real(dcu_j1_k1(2)))
         dcu(3,k1,1) = cmplx(real(dcu_1_k1(3)),real(dcu_j1_k1(3)))
      endif
      return
      end
!-----------------------------------------------------------------------
...
! mode numbers kx = 0, nx/2
  ...
  30    continue
      ...
      sum1 = sum1 + wp
   40 continue
      wf = real(nx)*real(ny)*sum1/affp                                         ! M. Touati
!	  wf = real(nx)*real(ny)*sum1                                              ! M. Touati
      return
      end
!-----------------------------------------------------------------------
